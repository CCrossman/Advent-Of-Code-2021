...
object Day04b extends AdventApp(4) {
...
//      }
  ...
  override def program(): Unit = {
    ...
    var lastWinner: Option[(Board,Int,Int)] = None
    var remainingBoards = boards

    selections.foreach(selection => {
      val nextIteration = scala.collection.mutable.ListBuffer.empty[Board]
      ...
      remainingBoards.foreach(board => {
        ...
        // check for victory
        if (board.isBingo) {
          lastWinner = Some((board,selection,board.sumOfUnmarkedValues * selection))
        } else {
          nextIteration.addOne(board)
        }
      })

      remainingBoards = nextIteration
    })
    val (board, selection, score) = lastWinner.get
